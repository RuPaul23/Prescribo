class ImageConstant {
  // Image folder path
  static String imagePath = 'assets/images';
  static String imagePath1 = 'assets\images\NavigationBar';

  // 1_Splash Screen images
  static String imgLogoNoBackground = '$imagePath/img_logo_no_background.png';

  // 2. Onboarding  images
  static String imgLogoColor1 = '$imagePath/img_logo_color_1.png';

  // 7_Login - Wrong Password images
  static String imgLocationRedA200 = '$imagePath/img_location_red_a200.svg';

  // 8_Login - Correct Password images
  static String imgLocationTeal40001 =
      '$imagePath/img_location_teal_400_01.svg';

  // 9_Login - Success images
  static String imgCheckmarkPrimary = '$imagePath/img_checkmark_primary.svg';

  // 10_Sign Up images
  static String imgLock = '$imagePath/img_lock.svg';

  // 12_Sign Up - Correct Password images
  static String imgIconCheckTeal40001 =
      '$imagePath/img_icon_check_teal_400_01.svg';

  // 15_Reset Password - Phone images
  static String imgIconCall = '$imagePath/img_icon_call.svg';

  // Home images
  static String imgNotification = '$imagePath/notification-bell.png';

  static String imgPrescription = '$imagePath/img_group_10.png';

  static String imgOrder = '$imagePath/img_group_9.png';

  static String imgCategory = '$imagePath/Category.png';

//Other
  static String imgMedicalPrescriptionRafiki =
      '$imagePath/img_medical_prescription_rafiki.png';

  static String imgMedicalPrescriptionPana =
      '$imagePath/img_medical_prescription_pana.png';

  static String imgPillsWhite = '$imagePath/img_pills_white.png';

  static String imgTick11 = '$imagePath/img_tick_1_1.png';

  static String imgPills1 = '$imagePath/img_pills_1.png';

  static String imgTick1 = '$imagePath/img_tick_1.png';

  static String imgRectangle460 = '$imagePath/img_rectangle_460.png';

  // Consult images
  static String imgPexelsCedricF = '$imagePath/img_pexels_cedric_f.png';

  static String imgSignal = '$imagePath/img_signal.svg';

  static String imgLinkedin = '$imagePath/img_linkedin.svg';

  static String imgPexelsEvelinaZhu5434019 =
      '$imagePath/img_pexels_evelina_zhu_5434019.png';

  static String imgPexelsThirdman5327580 =
      '$imagePath/img_pexels_thirdman_5327580.png';

  static String imgPexelsThirdman5327656 =
      '$imagePath/img_pexels_thirdman_5327656.png';

  static String imgPexelsAnthony = '$imagePath/img_pexels_anthony.png';

  // Articles images
  static String imgRectangle46087x138 =
      '$imagePath/img_rectangle_460_87x138.png';

  static String imgBookmarkWhiteA700 = '$imagePath/img_bookmark_white_a700.svg';

  static String imgRectangle458 = '$imagePath/img_rectangle_458.png';

  static String imgRectangle46087x113 =
      '$imagePath/img_rectangle_460_87x113.png';

  static String imgUnsplash86rvjm9zowy =
      '$imagePath/img_unsplash_86rvjm9zowy.png';

  static String imgUnsplashFepfs43yipe =
      '$imagePath/img_unsplash_fepfs43yipe.png';

  static String imgUnsplash5tymgag0wro =
      '$imagePath/img_unsplash_5tymgag0wro.png';

  // Remainder images
  static String imgIconChevronLeftWhiteA700 =
      '$imagePath/img_icon_chevron_left_white_a700.svg';

  static String imgAdd = '$imagePath/img_add.png';

  static String imgMorning = '$imagePath/img_morning.png';

  static String imgPills = '$imagePath/img_pills.png';

  static String imgRightArrow1 = '$imagePath/img_right_arrow_1.png';

  // Precription images
  static String imgArrowDownSign = '$imagePath/img_arrow_down_sign.png';

  static String imgMenubar = '$imagePath/menu-bar.png';

  static String imgImage23 = '$imagePath/img_image_23.png';

  // Profile images
  static String imgLogOut = '$imagePath/Danger Circle.svg';
  static String imgHelpCentre = '$imagePath/Chat.svg';
  static String imgAccount = '$imagePath/img_user_Profile.svg';
  static String imgPrescriptionAccount = '$imagePath/prescription-svgrepo-com.svg';
  static String imgSettings = '$imagePath/settings-svgrepo-com.svg';

                                          






  // Scanner images
  static String imgImage17 = '$imagePath/img_image_17.png';

  // Profile images
  static String imgTelevision = '$imagePath/img_television.svg';

  static String imgSettingsWhiteA700 = '$imagePath/img_settings_white_a700.svg';

  static String imgThumbsUp = '$imagePath/img_thumbs_up.svg';

  static String imgEllipse27 = '$imagePath/img_ellipse_27.png';

  static String imgCamera = '$imagePath/img_camera.svg';

  static String imgCameraProfile = '$imagePath/camera-svgrepo-com.svg';

  static String imgGroup33 = '$imagePath/img_group_33.png';

  static String imgSettings43x43 = '$imagePath/img_settings_43x43.svg';

  static String imgArrowRight = '$imagePath/img_arrow_right.svg';

  static String imgUserPrimary = '$imagePath/img_user_primary.svg';

  static String imgSettingsPrimary = '$imagePath/img_settings_primary.svg';

  static String imgSettingsPrimary43x43 =
      '$imagePath/img_settings_primary_43x43.svg';

  static String imgVideoCamera = '$imagePath/img_video_camera.svg';

  // Log Out - Pop Up images
  static String imgIcRoundLogout = '$imagePath/img_ic_round_logout.svg';

  // Common images
  static String imgIconChevronLeft = '$imagePath/img_icon_chevron_left.svg';

  static String imgCheckmark = '$imagePath/img_checkmark.svg';

  static String imgLocation = '$imagePath/img_location.svg';

  static String imgEye = '$imagePath/img_eye.svg';

  static String imgGoogle = '$imagePath/img_google.svg';

  static String imgApple = '$imagePath/img_apple.svg';

  static String imgCheckmarkTeal40001 =
      '$imagePath/img_checkmark_teal_400_01.svg';

  static String imgIconCheck = '$imagePath/img_icon_check.svg';

  static String imgAccount1 = '$imagePath/img_settings.svg';

  static String imgIconPassword = '$imagePath/img_icon_password.svg';

  static String imgPlaceholder = '$imagePath/img_placeholder.svg';

  static String imgIconEyeSlash = '$imagePath/img_icon_eye_slash.svg';

  static String imgCheckmarkTeal4000131x41 =
      '$imagePath/img_checkmark_teal_400_01_31x41.svg';

  static String imgIconChevronLeftOnprimary =
      '$imagePath/img_icon_chevron_left_onprimary.svg';

  static String imgArrowRightOpen = '$imagePath/arrowRight.png';

  static String imgRewind = '$imagePath/img_rewind.svg';

  static String imgDoctor = '$imagePath/img_group_8.png';

  static String imgBookmark = '$imagePath/img_bookmark.svg';

  static String imgGroup19 = '$imagePath/img_group_19.png';

  static String imgComponent1 = '$imagePath/img_component_1.svg';

  static String imageNotFound = 'assets/images/image_not_found.png';

  static String imgRightArrow = '$imagePath/RightGo.png';

  static String rightarrow = '$imagePath/right-arrow (2).png';

  static String imgSearch = '$imagePath/Search.png';

  //Navigation Bar
  static String imgHomeBar = '$imagePath/Home.png';
  static String imgPrescriptionBar = '$imagePath/Report.png';
  static String imgCalenderBar = '$imagePath/Calendar.png';
  static String imgProfile = '$imagePath/Profile.png';
  static String imgScanner = '$imagePath/Scanner.png';
}
